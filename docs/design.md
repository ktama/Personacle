...
**承認状況**: ADR-01, 02, 04, 05 は発注者承認済み。ADR-03, 06, 07 は承認済みADRから従属的に導かれる判断であり、提案ステータスで記載する(異議がなければ実装開始をもって承認とみなす)。
...
### ADR-06: 記憶生成・人格更新のタイミング [ステータス: 提案]
...
性格軸(初期案・要件9-4のプロトタイプ検証対象): `社交性` `共感性` `慎重さ` `自己主張` `明朗さ` の5軸。作成時は初期設定文からLLMが初期値を評定し、ユーザーが調整できる。**1セッションあたり変化量上限: 各軸±2**(設定値)。
...
1. `start_session(autonomous, [A, B], theme)` — A・B いずれかが他の active セッションに参加中なら BusyError(EC-08)
2. ターンループ: 発話者=A, B, A, … と交互。各ターンで (a) 停止フラグ検査 → 立っていれば終了(FR-14: 次の生成前に停止) (b) 発話者視点でプロンプト構築(相手の発話を相手の名前で提示、テーマを冒頭に) (c) ストリーミング生成・保存
3. 終了条件: ターン数が設定値に到達(EC-12) / 手動停止 / 連続生成失敗2回
4. 終了時に status=ended とし、**参加ペルソナそれぞれについて**後処理(フロー3)を実行 — A と B は同じ会話から各自の視点で別々の記憶・関係性変化を得る(FR-15)
...
| FR-18 | ペルソナ単位のJSONエクスポート/インポート(persona/trait/relationship/memory/personality_event を書き出し。会話履歴は選択制)。Could のため詳細は実装時 | 5.3 の schema_version をファイルにも記録 |
| FR-19 | 7章フロー2 のターンループを発話者選択ストラテジとして分離(2体=交互、3体以上=将来差し替え) | Could。拡張点のみ確保 |
...
| # | 論点 | 決定に必要な情報 | 決定者 |
| --- | --- | --- | --- |
| D-1 | 推奨モデルの確定(要件9-2の残り) | R-1/R-2 の PoC 結果 | 発注者(実装者が候補提示) |
| D-2 | 性格軸5軸と変化量上限の最終確定(要件9-4) | R-4 のプロトタイプ結果 | 発注者 |
| D-3 | 画面デザインの詳細(レイアウト、配色) | 実装時にプロトタイプで確認 | 発注者 |
| D-4 | 配布形態・ライセンス(要件9-5) | 依存ライセンス調査(実装後) | 発注者 |
